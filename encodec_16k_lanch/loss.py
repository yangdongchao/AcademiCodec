...
def reconstruction_loss(x, G_x, args, eps=1e-7):
    ...
    # for i in range(5, 12): # Encodec setting
        ...
        melspec = MelSpectrogram(
            sample_rate=args.sr, n_fft=s, hop_length=s//4, n_mels=64,
        ...
        loss = (
            (S_x - S_G_x).abs().mean() \
            + (((torch.log(S_x.abs()+eps) - torch.log(S_G_x.abs()+eps))**2).mean(dim=-2)**0.5).mean()
        ) / i
        L += loss
        # _l1_loss = (S_x - S_G_x).abs().mean()
        # _l2_loss = F.mse_loss(torch.log(S_x.abs()+eps), torch.log(S_G_x.abs()+eps))
        # L = L + _l1_loss + _l2_loss
        #print('i ,loss ', i, loss)
    #assert 1==2
    return L
...
def criterion_d(y_disc_r, y_disc_gen, fmap_r_det, fmap_gen_det,
    ...
    loss = (loss1/len(y_disc_gen) + loss2/len(y_df_hat_r) + loss3/len(y_ds_hat_r))/3.0

    return loss

def criterion_g(commit_loss, x, G_x, fmap_r, fmap_gen, y_disc_r, y_disc_gen, 
               y_df_hat_r, y_df_hat_g, fmap_f_r, fmap_f_g,
               y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g, args):
    adv_g_loss = adversarial_g_loss(y_disc_gen)
    feat_loss  = (feature_loss(fmap_r, fmap_gen) + sim_loss(y_disc_r, y_disc_gen) + \
                 feature_loss(fmap_f_r, fmap_f_g) + sim_loss(y_df_hat_r, y_df_hat_g) + \
                 feature_loss(fmap_s_r, fmap_s_g) + sim_loss(y_ds_hat_r, y_ds_hat_g))/3.0
...
def loss_g(codebook_loss, inputs, reconstructions, fmap_r, fmap_gen, 
           ...
           fmap_f_r, fmap_f_g, fmap_s_r, fmap_s_g, last_layer=None, is_training=True, args=None):
    """
    args:
    ...
    adv_loss = (adv_g_loss + adv_mpd_loss + adv_msd_loss)/3.0 # NOTE(lsx): need to divide by 3?
    feat_loss = feature_loss(fmap_r, fmap_gen) #+ sim_loss(y_disc_r, y_disc_gen) # NOTE(lsx): need logits?
    feat_loss_mpd = feature_loss(fmap_f_r, fmap_f_g) #+ sim_loss(y_df_hat_r, y_df_hat_g)
    feat_loss_msd = feature_loss(fmap_s_r, fmap_s_g) #+ sim_loss(y_ds_hat_r, y_ds_hat_g)
...
def loss_dis(y_disc_r_det, y_disc_gen_det, fmap_r_det, fmap_gen_det, 
    ...
    d_loss = disc_factor * criterion_d(y_disc_r_det, y_disc_gen_det, fmap_r_det, fmap_gen_det,
             y_df_hat_r, y_df_hat_g, fmap_f_r, fmap_f_g, y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g)
    return d_loss
