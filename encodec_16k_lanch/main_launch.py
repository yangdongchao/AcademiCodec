...
def get_args():
    ...
    parser.add_argument('--PATH', type=str, default='model_path', help='model save path')
...
def main_worker(local_rank, args):
    ...
    if args.distributed:
        soundstream = DDP(soundstream, device_ids=[args.local_rank], find_unused_parameters=find_unused_parameters) # device_ids=[args.local_rank], output_device=args.local_rank
        stft_disc = DDP(stft_disc, device_ids=[args.local_rank], find_unused_parameters=find_unused_parameters)
        msd = DDP(msd, device_ids=[args.local_rank], find_unused_parameters=find_unused_parameters)
        mpd = DDP(mpd, device_ids=[args.local_rank], find_unused_parameters=find_unused_parameters)
    ...
    for epoch in range(args.st_epoch, args.N_EPOCHS+1):
        ...
        for x in tqdm(train_loader):
            ...
            for optimizer_idx in [0,1]:  # we have two optimizer
                ...
                if optimizer_idx==0:
                    ...
                    total_loss_g, rec_loss, adv_g_loss, feat_loss, d_weight = loss_g(
                        ...
                        y_df_hat_r, y_df_hat_g, y_ds_hat_r, y_ds_hat_g,
            ...
            message = '<epoch:{:d}, iter:{:d}, total_loss_g:{:.4f}, adv_g_loss:{:.4f}, feat_loss:{:.4f}, rec_loss:{:.4f}, commit_loss:{:.4f}, loss_d:{:.4f}, d_weight: {:.4f}>'.format(epoch, 
                    k_iter, total_loss_g.item(), adv_g_loss.item(), feat_loss.item(), rec_loss.item(), \
